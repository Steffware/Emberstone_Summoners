...
- Mobs have a star rating. Base mobs are one star. The summon UI chooses a rating from 1 up to the current Emberstone level using `+` and `-` buttons, and renders the rating as gold `*` placeholders. Summon cost is `10 * (10 ** (rating - 1))`, so 1 star costs 10 Essence, 2 stars cost 100, and 3 stars cost `1e3`. Summoned mob base stats are multiplied by the mob rating, including Adaptability.
...
- The Summon and Sacrifice right window is the Sacrifice window. It has a mob dropdown populated from currently owned mobs, shows the selected mob's sprite/name/stars/type/Power/stats without progress bars, and has a `Sacrifice` button. Sacrificing grants `sqrt(sum(all mob stats))` Lifeforce, removes the mob from `owned_mobs`, and starts a 60-second sacrifice cooldown shown to the right of the Lifeforce gain label.
- The Mobs left window shows the owned mob pool as a 7-column grid at the current logical resolution, with `48x48` sprites centered in each occupied cell. Visible cell borders are drawn only for cells containing mob sprites. The grid extends vertically beyond the visible area, is clipped to the window interior, scrolls with mouse wheel when hovered, and has a draggable right-edge scrollbar.
- Mobs can be selected from the pool. The selected cell has a white outline; hovering another visible mob draws a second white outline. The Mobs right window displays the selected mob sprite, name, gold star rating, type, Power, stat descriptions, and per-stat progress bars. Power is the sum of all mob stats.
- Each mob stat has independent experience. For now each stat needs 10 XP to level. XP gained is multiplied by the mob's current Adaptability before being applied. Stat level-up gain is `2 ** (rating - 1)`, so 1-star mobs gain `+1`, 2-star mobs gain `+2`, and 3-star mobs gain `+4`. Adaptability keeps its decimal scale and gains `0.1 * stat_level_gain`.
